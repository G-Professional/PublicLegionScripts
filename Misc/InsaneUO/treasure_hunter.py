...
def findChestStatus(tchest):
    if API.Contents(tchest.Serial) == 0: #Chest is locked OR empty
        return 1
    if API.InJournalAny(["That appears to be trapped"]):
        return 2
    return 3
...
while not gump.IsDisposed: #Stop the script if the gump is closed
    ...
    if location != None:    
        if map != 0 and abs(API.Player.X - location[0]) <3 and abs(API.Player.Y - location[1]) <3 and status == 0 and not enemyrange(2):
